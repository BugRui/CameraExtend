                ...
                try {
                    ...
                    if (path.open() != null) {
                        if (path.getMedia().isCompressed()
                            ...
                            boolean isHasVideo = PictureMimeType.isHasVideo(path.getMedia().getMimeType());

                            if (isHasVideo) {
                                File result = new File(path.getPath());
                                newPath = result.getAbsolutePath();
                            } else {
                                File result = compress(context, path) == null ? new File(path.getPath()) : compress(context, path);
                                newPath = result.getAbsolutePath();
                            }
                        }
                    } else {
